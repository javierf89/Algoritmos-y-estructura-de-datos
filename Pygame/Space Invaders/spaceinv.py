            ...
            while self.running:
                ...
                if not self.game_mode == 'new high score':
                    ...
                    if keys[pygame.K_d]:
                        self.ship.move(1, self.screen_size)
                    if keys[pygame.K_w]:
                        self.ship.move(0, self.screen_size)
                    if keys[pygame.K_s]:
                        self.ship.move(0, self.screen_size)
...
class Ship:
        ...
        def move(self, direction, screen_size):
            ...
            if self.status in (0, 1):
                ...
                self.rect = pygame.Rect(self.position, self.size)
                self.shield_rect = pygame.Rect(self.position[0] - (self.shield_size[0] - self.size[0]) // 2,
                                            self.position[1] - (self.shield_size[1] - self.size[1]) // 2,
                                            self.shield_size[0], self.shield_size[1])
                                            
        def draw(self, screen):
            screen.blit(self.pic, (round(self.position[0]), round(self.position[1])))
            if self.shield:
...
def main_menu():
    while True:
        ...
        draw_text("Menú Principal", title_font, WHITE, screen, 250, 100)

        # Dibujar opciones
        option_font = pygame.font.Font(None, 50)
        draw_text("1. Jugar", option_font, WHITE, screen, 300, 250)
        draw_text("2. Configuración", option_font, WHITE, screen, 300, 320)
        draw_text("3. Salir", option_font, WHITE, screen, 300, 390)
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_1:
                    ...
                    # Lógica para la opción 2 (configuración)
                    settings_menu()
                    print("Configuración")
...
def settings_menu():
    ...
    player_lives = 3
    brightness = 100  # Porcentaje de brillo (inicializado al 100%)

    while running:
        screen.fill(BLACK)

        title_font = pygame.font.Font(None, 60)
        draw_text("Configuración", title_font, WHITE, screen, 250, 50)

        option_font = pygame.font.Font(None, 40)
        draw_text(f"1. Más Vidas ({player_lives})", option_font, WHITE, screen, 200, 150)
        draw_text(f"2. Brillo ({brightness}%)", option_font, WHITE, screen, 200, 220)
        draw_text("3. Volver al menú principal", option_font, WHITE, screen, 200, 290)

        pygame.draw.rect(screen, WHITE, (180, 140 + selected_option * 70, 20, 20), 2)
            ...
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_UP:
                    ...
                    if selected_option == 0:
                        # Lógica para la "Más Vidas"
                        player_lives += 1
                        print(f"¡Ahora tienes {player_lives} vidas!")
                    elif selected_option == 1:
                        # Lógica para la opción de brillo
                        if pygame.key.get_mods() & pygame.KMOD_SHIFT:  # Si se mantiene presionada la tecla Shift
                            brightness = max(0, brightness - 10)  # Disminuir el brillo en un 10%
                        else:
                            brightness = min(100, brightness + 10)  # Aumentar el brillo en un 10%
